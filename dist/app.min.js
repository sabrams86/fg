...
angular.module('jkuri.bookingcalendar', []).factory('MultiCalendarService', ['$http', function ($http) {
  ...
  return {
    ...
    template: '<div class="ng-multi-calendar">' + '  <div class="ng-multi-calendar-select">' + '    <label>{{ numMonthsText }}' + '    <select ng-model="num" ng-options="n for n in displayMonthsOpts">' + '    </select>' + '    </label>' + '  </div>' + '  <div class="ng-multi-calendar-loader" ng-show="loading"><div class="uil-ring-css"><div></div></div></div>' + '  <div class="left">' + '    <i class="fa fa-angle-left" ng-click="prevMonth()"></i>' + '  </div>' + '  <div class="calendar" ng-repeat="c in calendars">' + '    <div class="month-name"><span ng-bind="c.dateValue" class="date"></span></div>' + '    <div class="day-names">' + '      <span ng-repeat="dn in dayNames">' + '        <span ng-bind="dn"></span>' + '      </span>' + '    </div>' + '    <div class="days">' + '      <span ng-repeat="d in c.days">' + '        <span class="day" ng-click="selectDate($event, d)" ng-class="{busy: d.busy, past: d.past_day, today: d.is_today, selected: d.selected}">{{ d.day }}</span>' + '      </span>' + '    </div>' + '  </div>' + '  <div class="right">' + '    <i class="fa fa-angle-right" ng-click="nextMonth()"></i>' + '  </div>' + '</div>'
  };
}]);
...
app.controller('ShowContractsController', ['$scope', '$location', '$cookies', 'AuthService', 'ItemService', 'ContractService', 'UserService', 'PaymentService', function ($scope, $location, $cookies, AuthService, ItemService, ContractService, UserService, PaymentService) {
  if (!$scope.userLoggedIn) $location.path('/');
